...
(async function loadAndRenderMap() {
    const mapTemplateId = $('#mapTemplateId').val();;

    const map = await loadMap(mapTemplateId);
...
async function loadMap(mapTemplateId) {
    const mapTemplatePath = `/uploads/maptemplates/${mapTemplateId}/File.json?nocache=${new Date().getTime()}`;

    try {
        const response = await fetch(mapTemplatePath);
